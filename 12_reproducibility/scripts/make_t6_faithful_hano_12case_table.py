...
def main() -> None:
    ...
    missing = []
    for case in CASES:
        path = REPORT_DIR / f"level4_t6_faithful_hano_12case_{case}.json"
        if not path.exists():
            missing.append(case)
            continue
        payload = json.loads(path.read_text(encoding="utf-8"))
        result = payload["results"]["hano_faithful"]
    ...
    if missing:
        raise FileNotFoundError(f"missing faithful HANO 12-case result JSON for: {', '.join(missing)}")

    out_base = REPORT_DIR / "level4_t6_faithful_hano_12case_main_table"
...
def _write_markdown(path: Path, rows: list[dict[str, str]], metadata: list[tuple[str, str, dict]]) -> None:
    headers = list(rows[0])
    lines = [
        "# Level-4 T6/QP Faithful HANO Complete 12-Case Baseline",
        "",
        "All rows use the faithful HANO strain-stress spectral-window operator, strict "
        "monotonic-to-cyclic training/testing, 50 epochs, five seeds, T=8, T6 elements, "
        "and the same FEM residual/energy evaluation audit used by the HistoryGNO tables.",
        "",
        "| " + " | ".join(headers) + " |",
    ...
    for case, source, meta in metadata:
        fem_training = meta.get("fem_training", {})
        lines.append(
            ...
            f"fem_training_weight={fem_training.get('residual_weight', 0.0)}"
        )
    lines.append("")
