...
def main() -> None:
    ...
    metadata = []
    for case in CASES:
        path = REPORT_DIR / f"level4_t6_qp_allstep_representative_{case}.json"
        ...
        rows.append(row)
        metadata.append((case, payload["metadata"]))

    out_base = REPORT_DIR / "level4_t6_qp_allstep_representative_table"
...
def _write_markdown(path: Path, rows: list[dict[str, str]], metadata: list[tuple[str, dict]]) -> None:
    ...
    for case, meta in metadata:
        lines.append(
            f"- {case}: `{meta['data_root']}`, epochs={meta['epochs']}, "
