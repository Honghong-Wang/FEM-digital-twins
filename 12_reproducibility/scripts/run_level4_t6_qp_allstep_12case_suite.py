...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run the complete 12-case Level-4 T6/QP all-step FEM audit suite. "
            "Each case uses QP-thermo-hard HistoryGNO, monotonic training, cyclic testing, "
            "50 epochs, five seeds, and all-step matrix-free residual/energy audit loss."
        )
    )
    ...
    for case in cases:
        ...
        stem = f"level4_t6_qp_allstep_12case_{case}"
        ...
        print(f"[run] {case}: epochs={args.epochs}, seeds={args.seeds}, device={args.device}")
